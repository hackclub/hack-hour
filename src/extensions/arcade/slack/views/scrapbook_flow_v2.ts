...
export class Scrapbook {
    public static async start(timestamp: string): Promise<KnownBlock[]> {
        return [
            {
                "type": "section",
                "text": {
                    "type": "mrkdwn",
                    "text": "I saw your scrapbook post! Let's get your hours banked in! :yay:"
                }
            },
            {
                "type": "divider"
    ...
    public static async select(timestamp: string, pages: number): Promise<ModalView> {
        return {} as any;
    }
    ...
    //                         "text": {
    //                             "type": "plain_text",
    //                             "text": `${goal.name.substring(0, 30)}\n${goal.sessions.length} sessions | Total Hours: ${formatHour(goal.sessions.reduce((acc, session) => acc + session.elapsed, 0))}`,
    //                             "emoji": true
    //                         },
    //                         "value": goal.id
    //                     }
